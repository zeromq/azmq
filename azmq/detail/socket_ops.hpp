...
struct socket_ops
{
    ...
    using stream_descriptor =
      std::unique_ptr<posix_sd_type, stream_descriptor_close>;
#else
    ...
    static stream_descriptor
    get_stream_descriptor (boost::asio::io_service &io_service,
                           socket_type &socket,
                           boost::system::error_code &ec)
    {
        ...
        if (rc < 0)
...
#if !defined BOOST_ASIO_WINDOWS
            res.reset (
              new boost::asio::posix::stream_descriptor (io_service, handle));
#else
            ...
            handle = ::WSASocket (pi.iAddressFamily /*AF_INET*/,
                                  pi.iSocketType /*SOCK_STREAM*/,
                                  pi.iProtocol /*IPPROTO_TCP*/, &pi, 0, 0);
            res.reset (new boost::asio::ip::tcp::socket (
              io_service, boost::asio::ip::tcp::v4 (), handle));
#endif
        }
        return res;
    }
    ...
    cancel_stream_descriptor (stream_descriptor &sd,
                              boost::system::error_code &ec)
    {
    ...
    static boost::system::error_code
    bind (socket_type &socket, endpoint_type &ep, boost::system::error_code &ec)
    {
        BOOST_ASSERT_MSG (socket, "invalid socket");
        static const std::regex simple_tcp ("^tcp://.*:(\\d+)$");
        static const std::regex dynamic_tcp (
          "^(tcp://.*):([*!])(\\[(\\d+)?-(\\d+)?\\])?$");
        std::smatch mres;
        int rc = -1;
        if (std::regex_match (ep, mres, simple_tcp)) {
            ...
            auto first = first_str.empty ()
                           ? static_cast<uint16_t> (dynamic_port::first)
                           : boost::lexical_cast<uint16_t> (first_str);
            auto last = last_str.empty ()
                          ? static_cast<uint16_t> (dynamic_port::last)
                          : boost::lexical_cast<uint16_t> (last_str);
            uint16_t port = first;
            if (opcode[0] == '!') {
                static boost::random::taus88 gen;
                boost::random::uniform_int_distribution<> port_range (port,
                                                                      last);
                port = port_range (gen);
            }
    ...
    {
        auto rc =
          zmq_setsockopt (socket.get (), opt.name (), opt.data (), opt.size ());
        if (rc < 0)
            ec = make_error_code ();
        return ec;
    }
    ...
    get_option (socket_type &socket, Option &opt, boost::system::error_code &ec)
    {
        ...
        auto rc =
          zmq_getsockopt (socket.get (), opt.name (), opt.data (), &size);
        if (rc < 0)
            ec = make_error_code ();
        return ec;
    }

    static int get_events (socket_type &socket, boost::system::error_code &ec)
    {
        BOOST_ASSERT_MSG (socket, "invalid socket");
        int evs = 0;
    ...
    {
        BOOST_ASSERT_MSG (socket, "Invalid socket");
        auto rc = zmq_msg_send (const_cast<zmq_msg_t *> (&msg.msg_),
                                socket.get (), flags);
        if (rc < 0) {
            ec = make_error_code ();
            return 0;
    ...
    static size_t send (package const &pkg,
                        socket_type &socket,
                        flags_type flags,
                        boost::system::error_code &ec)
    {
        ...
        auto last =
          std::distance (std::begin (pkg.messages_), std::end (pkg.messages_))
          - 1;
        decltype (last) index = 0u;

        for (const auto &message : pkg.messages_) {
            auto f = index == last ? flags : flags | ZMQ_SNDMORE;

            index++;
            auto rc = zmq_msg_send (const_cast<zmq_msg_t *> (&message.msg_),
                                    socket.get (), f);
            if (rc < 0) {
                ec = make_error_code ();
                return ret;
    ...
    static auto send (ConstBufferSequence const &buffers,
      ...
      typename boost::enable_if<
        boost::has_range_const_iterator<ConstBufferSequence>,
        size_t>::type
    {
        size_t res = 0;
        auto last =
          std::distance (std::begin (buffers), std::end (buffers)) - 1;
        decltype (last) index = 0u;
        for (auto it = std::begin (buffers); it != std::end (buffers);
             ++it, ++index) {
            auto f = index == last ? flags : flags | ZMQ_SNDMORE;
    ...
    {
        BOOST_ASSERT_MSG (socket, "Invalid socket");
        auto rc = zmq_msg_recv (const_cast<zmq_msg_t *> (&msg.msg_),
                                socket.get (), flags);
        if (rc < 0) {
            ec = make_error_code ();
            return 0;
    ...
    static size_t receive (package &pkg,
                           socket_type &socket,
                           flags_type flags,
                           boost::system::error_code &ec)
    {
        ...
        while (more != 0) {
            ...
            num = zmq_msg_recv (&buffer, socket.get (), flags);
            if (num < 0) {
                ec = make_error_code ();
                break;
            }

            more = zmq_msg_more (&buffer);
            auto sz = zmq_msg_size (&buffer);
            pkg.messages_.emplace_back (
              boost::asio::buffer (zmq_msg_data (&buffer), sz));
    ...
    static auto receive (MutableBufferSequence const &buffers,
                         ...
                         boost::system::error_code &ec) ->
      typename boost::enable_if<
        boost::has_range_const_iterator<MutableBufferSequence>,
        size_t>::type
    {
        size_t res = 0;
        message msg;
    ...
    static size_t flush (socket_type &socket, boost::system::error_code &ec)
    {
        size_t res = 0;
        message msg;
    ...
    static std::string
    monitor (socket_type &socket, int events, boost::system::error_code &ec)
    {
        BOOST_ASSERT_MSG (socket, "Invalid socket");
